...
export async function dispatchTransferRequest(req, res) {
  try {
    ...
    const transfer = await dispatchTransfer(transferId, req.user.userId);
    ...
    if (
      ...
      error.message === "Source inventory not found" ||
      error.message === "Insufficient available stock" ||
      error.message === "Only requested transfers can be dispatched"
    ) {
      return res.status(400).json({
        message: error.message,
      });
...
export async function receiveTransferRequest(req, res) {
  try {
    ...
    const transfer = await receiveTransfer(transferId, req.user.userId);
    ...
    if (
      ...
      error.message === "Only dispatched transfers can be received"
    ) {
      return res.status(400).json({
        message: error.message,
      });
